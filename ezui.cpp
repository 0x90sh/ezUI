...
public:
    HWND findWindowApproximate(const std::string& partialName);
...
public:
    ...
    void drawCircle(float centerX, float centerY, float radius, const Color& color, int segments = 48);
    void drawRoundedRectangle(float x, float y, float width, float height, float radius, const Color& color, int segments = 48);
...
HWND WindowHijacker::findWindowApproximate(const std::string& partialName) {
...
void DX11Renderer::createRenderTarget() {
    ...
    if (FAILED(hr)) {
        std::cerr << "Failed to get the swap chain's back buffer! HRESULT: " << std::hex << hr << std::endl;
        return;
    }
...
void DX11Renderer::createBlendState() {
    ...
    if (FAILED(hr)) {
        std::cerr << "Failed to create blend state! HRESULT: " << std::hex << hr << std::endl;
        return;
    }
...
void DX11Renderer::drawRectangle(float x, float y, float width, float height, const Color& color) {
    if (!d3dDevice || !d3dContext) {
        std::cerr << "Device or Context not initialized!" << std::endl;
        return;
    }
    ...
    if (FAILED(hr)) {
        std::cerr << "Failed to map vertex buffer! HRESULT: " << std::hex << hr << std::endl;
        return;
    }
...
void DX11Renderer::drawCircle(float centerX, float centerY, float radius, const Color& color, int segments) {
    ...
    if (FAILED(hr)) {
        std::cerr << "Failed to map index buffer! HRESULT: " << std::hex << hr << std::endl;
        return;
    }
...
int main() {
    ...
    HWND hwnd = hijacker.findWindowApproximate("yourwindowtohijack");
    ...
    while (msg.message != WM_QUIT) {
        ...
        renderer.drawRectangle(50.0f, 50.0f, 100.0f, 100.0f, Color(1.0f, 0.0f, 0.0f, 1.0f));
        renderer.drawTriangle(175.0f, 50.0f, 275.0f, 50.0f, 225.0f, 150.0f, Color(1.0f, 0.0f, 0.5f, 1.0f));
        renderer.drawCircle(350.0f, 100.0f, 50.0f, Color(0.0f, 1.0f, 0.0f, 1.0f));
        renderer.drawRoundedRectangle(425.0f, 50.0f, 200.0f, 100.0f, 20.0f, Color(0.0f, 0.0f, 1.0f, 1.0f));
        renderer.present();
    }
